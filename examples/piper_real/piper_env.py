...
def kalman_filter(data):
    """
    对二维数组的每一列数据进行 Kalman 滤波，
    并将每一列的第一行数据作为初始状态。

    :param data: 输入的二维 numpy 数组（如 64x7）
    :return: 滤波后的 numpy 数组
    """
    ...
    for col in range(data.shape[1]):
        initial_state = data[0, col]  # 每一列的第一行作为初始值
        kf = KalmanFilter(initial_state_mean=initial_state, n_dim_obs=1)
        smoothed_data[:, col] = kf.em(data[:, col]).smooth(data[:, col])[0].flatten()
...
def model_inference(args, config, ros_operator):
    ...
    pre_action[:14] = np.array(
        ...
        [-0.00133514404296875, 0.00247955322265625, 0.01583099365234375, -0.032616615295410156, -0.00286102294921875,
        ...
        while True and not rospy.is_shutdown():
            ...
            while t < max_publish_step and not rospy.is_shutdown():
                ...
                if t % chunk_size == 0:
                    ...
                    # use kalman
                    # action_buffer = kalman_filter(action_buffer)
